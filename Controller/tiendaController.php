...
//REDES corregir 
    //----CREATE
public function createRedesTiendas(ServerRequest $request,$membresia,$id_negocio,$email){
        //Verifico si el negocio existe
        $negocio = new Tiendas;
        $negocio=$negocio->getOne($id_negocio);
        if(empty($negocio)){
        ...
        if($membresia && $membresia = 1){ //???
            ...
            $redes = new Redes;
            return new JsonResponse($redes->create($data_arr,$id_negocio));
        }
    }
...
public function deleteRedes($id){
...
public function redesUpdate(ServerRequest $request, $id){
        ...
        $redes = new Redes;
        return new JsonResponse($redes->update($id_al,$data_arr));
    }
}
